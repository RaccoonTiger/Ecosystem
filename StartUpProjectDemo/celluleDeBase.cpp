...
celluleDeBase::celluleDeBase(double lifeExpectancy, QPointF const& initialPosition, qreal initialOrientationDegrees, qreal initialSpeed, qreal scale, QBrush const& brush, QGraphicsItem* parent)
...
QBrush celluleDeBase::getColor()
{
	return mBrush;
}

qreal celluleDeBase::getSpeed()
{
	return mSpeed;
}

void celluleDeBase::setColor(QBrush const& brush)
{
	mBrush = brush;
}

void celluleDeBase::setSpeed(qreal speed)
{
	mSpeed = speed;
}

QRectF celluleDeBase::boundingRect() const
{
	return mShape.boundingRect();
}

void celluleDeBase::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
...
void celluleDeBase::advance(int phase)
{
	//
...
bool celluleDeBase::isAlive() const
{
	return mAge <= mLifeExpectancy;
}

void celluleDeBase::split()
{
}

void celluleDeBase::drawChampVision(const QPointF& center, qreal rx, qreal ry)
{
}

qreal celluleDeBase::warp(qreal value, qreal begin, qreal end) {
	qreal const width = end - begin;
	return value - qFloor((value - begin) / width) * width;
}

void celluleDeBase::warp(QPointF& point) {
